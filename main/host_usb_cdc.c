...
static const char *TAG = "HOST_USB-CDC";
...
static void host_usb_cdc_init(void)
{
    ...
    ESP_LOGI(TAG, "Waiting for CDC devices. Press Boot button to quit.");
    ...
    while (running) {
        ...
        case APP_DEVICE_CONNECTED: {
            ...
            if (slot < 0) {
                ESP_LOGW(TAG, "No free slots for new CDC device (max %d)", MAX_CDC_DEVICES);
                continue;
            }
        ...
        case APP_DEVICE_DISCONNECTED: {
            ESP_LOGI(TAG, "Device disconnected from slot %d", msg.data.device_slot);
            free_cdc_device(msg.data.device_slot);
            break;
        }

        case APP_QUIT: {
            ESP_LOGI(TAG, "Exiting example");
        ...
        default:
            ESP_LOGW(TAG, "Unknown message ID: %d", msg.id);
            break;
        }
    }

    ESP_LOGI(TAG, "\t- Exit completed");
